(function (global, factory) {
  ...
  (global = global || self, global.Serina = factory());
  ...
  */
  ...
  var WeekDay = (function () {
      ...
      WeekDay.parseText = function (text) {
          var _this = this;
          var pattern = "((" + RELATIVE.FUTURE_WORDS + "|" + RELATIVE.PAST_WORDS + ") )?" + WEEKDAY.ALL;
          var matchForWeekdays = matchPattern(text, pattern);
          if (!matchForWeekdays)
              return null;
          return matchForWeekdays.map(function (elem) { return _this.parseWeekdayMatches(text, elem); });
      };
      WeekDay.matchWeekdays = function (text) {
          var matched = matchPattern(text, WEEKDAY.ALL);
          return matched ? matched : null;
      };
      WeekDay.parseWeekdayMatches = function (text, matchedWeekday) {
      ...
      WeekDay.convertWeekdayMatchToDate = function (matchingText) {
          var weekday = null;
          var todayInWeekday = DateTime.local().weekday;
          if (contains(matchingText, WEEKDAY.MONDAY))
              weekday = 1;
          if (contains(matchingText, WEEKDAY.TUESDAY))
              weekday = 2;
          if (contains(matchingText, WEEKDAY.WEDNESDAY))
              weekday = 3;
          if (contains(matchingText, WEEKDAY.THURSDAY))
              weekday = 4;
          if (contains(matchingText, WEEKDAY.FRIDAY))
              weekday = 5;
          if (contains(matchingText, WEEKDAY.SATURDAY))
              weekday = 6;
          if (contains(matchingText, WEEKDAY.SUNDAY))
              weekday = 7;
          if (weekday <= todayInWeekday)
              weekday += 7;
          if (!weekday)
              return null;
          if (contains(matchingText, RELATIVE.PAST_WORDS + " " + WEEKDAY.ALL))
              weekday -= 7;
  ...
  ***************************************************************************** */
  ...
  var Day = (function () {
      ...
      Day.parseText = function (text) {
          var _this = this;
          var pattern = "(" + FILLER_WORD.DAY + ")?" + DAY.ALL;
          var matchForDates = matchPattern(text, pattern);
          if (!matchForDates)
              return null;
          return matchForDates.map(function (elem) { return _this.parseDateMatches(text, elem); });
      };
      Day.parseDateMatches = function (text, matchedDate) {
          var replaceMatch = text.toLowerCase().replace(matchedDate, '');
          return {
              text: trimWhiteSpaces(replaceMatch),
      ...
      Day.convertMatchToDateObj = function (matchingText) {
          var today = DateTime.local();
          var day = null;
          var month = DateTime.local().month;
          if (contains(matchingText, DAY.ALL)) {
              var _a = __read(matchPattern(matchingText, DAY.ALL), 1), matchedDay = _a[0];
  ...
  var Year = (function () {
      ...
      Year.parseText = function (text) {
          var _this = this;
          var pattern = "(" + FILLER_WORD.YEAR + ")?" + YEAR.ALL;
          var matches = matchPattern(text, pattern);
          if (!matches)
              return null;
          return matches.map(function (match) { return _this.parseDateMatches(text, match); });
      };
      Year.parseDateMatches = function (text, matchedDate) {
          var replaceMatch = text.toLowerCase().replace(matchedDate, '');
          return {
              text: trimWhiteSpaces(replaceMatch),
              dateTime: this.convertMatchToDateObj(matchedDate),
      ...
      Year.convertMatchToDateObj = function (matchingText) {
          var year = DateTime.local().year;
          if (contains(matchingText, YEAR.ALL)) {
              var _a = __read(matchPattern(matchingText, YEAR.ALL), 1), matchedDay = _a[0];
  ...
  var serina = function (text) {
      ...
      var weekdays = WeekDay.parseText(text);
      if (weekdays && weekdays.length)
          parsedData.matches = parsedData.matches.concat(weekdays);
      var day = Day.parseText(text);
      if (day && day.length)
          parsedData.matches = parsedData.matches.concat(day);
      var year = Year.parseText(text);
      if (year && year.length)
          parsedData.matches = parsedData.matches.concat(year);
      if (parsedData.matches.length)
