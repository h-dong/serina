...
} from 'parsers/partialDates/partialDates.helpers';
...
describe('Partial Dates Helpers', () => {
  ...
  describe('getNextMonthIfDayIsInThePast()', () => {
    ...
    test('wraps to January when the next month would pass December', () => {
      vi.setSystemTime(new Date('2019-12-29T15:48:12Z'));
      expect(getNextMonthIfDayIsInThePast('1')).toBe(1);
      vi.setSystemTime(new Date('2019-06-29T15:48:12Z'));
    });
  });
  ...
  describe('partialDateStringToDayMonthYear', () => {
    ...
    test('uses en-US month/day order for ambiguous numeric partial dates', () => {
      const parsedText = partialDateStringToDayMonthYear('02/10', 'en-US');
      expect(parsedText).toEqual(new Date('2020-02-10T00:00:00.000Z'));
    });

    test('uses en-GB day/month order for ambiguous numeric partial dates', () => {
      const parsedText = partialDateStringToDayMonthYear('02/10', 'en-GB');
      expect(parsedText).toEqual(new Date('2019-10-02T00:00:00.000Z'));
    });
  });
  ...
  describe('partialDateStringToDateObj()', () => {
    ...
    test('passes locale through after filler words are removed', () => {
      const parsedText = partialDateStringToDateObj('on 02/10', 'en-US');
      expect(parsedText).toEqual(new Date('2020-02-10T00:00:00.000Z'));
    });
  });
});
