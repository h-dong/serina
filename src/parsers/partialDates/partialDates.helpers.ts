...
function isAmbiguousNumericPartialDate(date: string): boolean {
  return contains(date, PARTIAL_DATES.NUM_DAY_MONTH) && contains(date, PARTIAL_DATES.NUM_MONTH_DAY);
}

export function partialDateStringToDayMonthYear(date: string, locale?: string): Date {
  let day: string;
  let month: string;
  let year: string;
  ...
  if (contains(date, PARTIAL_DATES.NUM_MONTH_YEAR)) {
    ...
    day = '1';
  } else if (isAmbiguousNumericPartialDate(date) && getDateOrder(locale) === 'MDY') {
    dividerRegex = new RegExp(DATES.NUM_DIVIDER, 'g');
...
export function partialDateStringToDateObj(matchingText: string, locale?: string): Date {
  const removedFillerWords = remove(matchingText, DATES.FILLER_WORDS);
  return partialDateStringToDayMonthYear(removedFillerWords, locale);
}
