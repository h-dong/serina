...
describe('dayLight', () => {
  ...
  describe('plus() operation', () => {
    ...
    test.each([
      ...
      { text: 'millisecond', operation: dayLite().plus(1, 'millisecond').millisecond, expected: 1 },
      ...
      { text: 'week', operation: dayLite().plus(1, 'week').day, expected: 28 },
    ...
    test('throws for unknown unit', () => {
      expect(() => dayLite().plus(1, 'unknown' as never)).toThrow('Cannot perform .plus() operation with unknown unit');
    });
  });
  ...
  describe('minus() operation', () => {
    ...
    test.each([
      ...
      { text: 'millisecond', operation: dayLite().minus(1, 'millisecond').millisecond, expected: 999 },
      ...
      { text: 'week', operation: dayLite().minus(1, 'week').day, expected: 13 },
    ...
    test('throws for unknown unit', () => {
      expect(() => dayLite().minus(1, 'unknown' as never)).toThrow(
        'Cannot perform .minus() operation with unknown unit',
      );
    });
  });
  ...
  describe('set() operation', () => {
    ...
    test.each([
      ...
      { text: 'day below range', operation: dayLite().set({ day: 0 }).day, expected: 1 },
      { text: 'day above range', operation: dayLite().set({ day: 99 }).day, expected: 30 },
      { text: 'weekday', operation: dayLite().set({ weekday: 1 }).weekday, expected: 1 },
    ])('should return correct $name', ({ operation, expected }) => {
  ...
  describe('next() and previous() operations', () => {
    vi.useFakeTimers().setSystemTime(new Date('2022-10-21T14:55:35.000Z'));
    ...
    test.each([
      { text: 'millisecond', operation: dayLite().next(1, 'millisecond').millisecond, expected: 1 },
      { text: 'second', operation: dayLite().next(1, 'second').second, expected: 36 },
      { text: 'minute', operation: dayLite().next(1, 'minute').minute, expected: 56 },
      { text: 'hour', operation: dayLite().next(1, 'hour').hour, expected: 15 },
      { text: 'day', operation: dayLite().next(1, 'day').day, expected: 22 },
      { text: 'week', operation: dayLite().next(1, 'week').day, expected: 28 },
      { text: 'month', operation: dayLite().next(1, 'month').month, expected: 11 },
      { text: 'year', operation: dayLite().next(1, 'year').year, expected: 2023 },
    ])('should move next $text', ({ operation, expected }) => {
      expect(operation).toEqual(expected);
    });

    test.each([
      { text: 'millisecond', operation: dayLite().previous(1, 'millisecond').millisecond, expected: 999 },
      { text: 'second', operation: dayLite().previous(1, 'second').second, expected: 34 },
      { text: 'minute', operation: dayLite().previous(1, 'minute').minute, expected: 54 },
      { text: 'hour', operation: dayLite().previous(1, 'hour').hour, expected: 13 },
      { text: 'day', operation: dayLite().previous(1, 'day').day, expected: 20 },
      { text: 'week', operation: dayLite().previous(1, 'week').day, expected: 14 },
      { text: 'month', operation: dayLite().previous(1, 'month').month, expected: 9 },
      { text: 'year', operation: dayLite().previous(1, 'year').year, expected: 2021 },
    ])('should move previous $text', ({ operation, expected }) => {
      expect(operation).toEqual(expected);
    });

    test('prev() aliases previous()', () => {
      expect(dayLite(new Date('2022-10-21T14:55:35.000Z')).prev(1, 'day').day).toBe(20);
    });

    test('throws for unknown next unit', () => {
      expect(() => dayLite().next(1, 'unknown' as never)).toThrow('Cannot perform .next() operation with unknown unit');
    });

    test('throws for unknown previous unit', () => {
      expect(() => dayLite().previous(1, 'unknown' as never)).toThrow(
        'Cannot perform .prev() or .previous() operation with unknown unit',
      );
    });
  });
  ...
  describe('start()', () => {
    ...
    test('throws for unknown unit', () => {
      expect(() => dayLite().start('unknown' as never)).toThrow(
        'Cannot perform .start() or .startOf() operation with unknown unit',
      );
    });
  });
  ...
  describe('end()', () => {
    ...
    test('throws for unknown unit', () => {
      expect(() => dayLite().end('unknown' as never)).toThrow(
        'Cannot perform .end() or .endOf() operation with unknown unit',
      );
    });
  });
});
